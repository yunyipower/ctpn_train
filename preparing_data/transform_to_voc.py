...
global_outdir = '/tmp/TEXTVOC_ICDAR_2015/VOC2007'
...
def generate_xml(name, lines, img_size, class_sets, doncateothers=True):
    ...
    for line in lines:
        ...
        occlusion = int(0)
        x1, y1, x2, y2, xside = int(float(splitted_line[1]) + 1), int(float(splitted_line[2]) + 1), \
        ...
        append_xml_node_attr('xside', parent=bb, text=str(xside))
...
if __name__ == '__main__':
    _draw = bool(0)
    _dest_label_dir, _dest_img_dir, _dest_set_dir = build_voc_dirs(global_outdir)
    _doncateothers = bool(1)
    for dset in ['train']:
        _labeldir = 'label_tmp_icdar_2015'
        _imagedir = 're_image_icdar_2015'
