...
class TextProposalDetector:
    ...
    def detect(self, im, mean):
        ...
        print "TextProposalDetector.detect: im_data shape: {}".format(im_data.shape)
        print "TextProposalDetector.detect: data shape: {}".format(im_data[np.newaxis, :].shape)
        ...
        xside=self.caffe_model.blob("anchor_bbox_xside_pred")
        return rois, scores, xside
...
class TextDetector:
    ...
    def detect(self, im):
        ...
        """
        text_proposals, scores, xsides=self.text_proposal_detector.detect(im, cfg.MEAN)
        keep_inds=np.where(scores>cfg.TEXT_PROPOSALS_MIN_SCORE)[0]
        text_proposals, scores, xsides=text_proposals[keep_inds], scores[keep_inds], xsides[keep_inds]

        sorted_indices=np.argsort(scores.ravel())[::-1]
        text_proposals, scores, xsides=text_proposals[sorted_indices], scores[sorted_indices], xsides[sorted_indices]
        ...
        text_lines=self.text_proposal_connector.get_text_lines(text_proposals, scores, xsides, im.shape[:2])
